...
public:
  ...
  virtual const Key& key() const {//выжаёт ключ узла. Ключ - уникальный номер узла. То есть у узла должно быть 4 характристики: ключ, значение, следующий, предыдущий узел и количество слоёв. Пока плохо понял, указывать ли соседей на каждом слое или как.
    assert(pCurrent != nullptr);//кидает ошибку при логическом нуле внутри функции
    return pCurrent->key();
  };

  virtual const Value& value() const {//выжаёт значение узла
